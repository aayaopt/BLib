package futurenavi.lib.erecyclerview.swiperecyclerview.touch;
...
/**
 * Description: 默认条目touch处理回调类，主要包括条目的移动，可折叠，长按拖拽等以及相关监听
 * @author Li Jianying
 * @version 2.0
 * @since 2016/8/19
 */
public class DefaultItemTouchHelperCallback extends ItemTouchHelper.Callback {
    ...
    /**
     * 获取条目移动时刻的相关状态，根据设置监听的状态来决定条目是否可以拖拽，滑动等
     * @param recyclerView
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        //回调适配器里面的方法，让其刷新数据及界面。
        if (onItemMoveListener != null)
            onItemMoveListener.onItemSwipe(viewHolder.getAdapterPosition(),direction);
    }
}